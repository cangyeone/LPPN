...
def main(args):
    stride = args.stride
    nchannel = args.fature 
    model_name = f"ckpt/{stride}-{nchannel}.wave"
    data_tool = utils.Data(file_name=args.input, batch_size=32, n_thread=1, strides=8, n_length=3072)
    ...
    optim = torch.optim.Adam(model.parameters(), args.lr, weight_decay=0.0)
    count = 0 
    for varkey in model.state_dict():
        ...
        for i in var.shape:
            c *= i 
        count += c 
    print("number of trainning parameters", count)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--device', default="cuda:0", help="runining device")
